...
{
    // Convert dravite:// to dravite:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("dravite://"))
    {
        uri.replace(0, 12, "dravite:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Dravite.lnk";
}
...
{
    return GetAutostartDir() / "Dravite.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Dravite-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Dravite-qt [" + tr("command-line options") + "]                     " + "\n";
