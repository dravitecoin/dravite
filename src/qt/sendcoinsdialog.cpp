    ...
    model(0)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a Dravite address (e.g. DPpauUHW4ksipzUQhKKVAxscM6UonAAjhX)"));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if (ui->entries->count() > 1 && ui->splitBlockCheckBox->checkState() == Qt::Checked)
    {
        ...
        ui->splitBlockCheckBox->setCheckState(Qt::Unchecked);
        QMessageBox::warning(this, tr("Send Coins"),
            tr("The split block feature does not work with multiple addresses. Please try again."),
            QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
...
void SendCoinsDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        ...
        else if (!CBitcoinAddress(text.toStdString()).IsValid())
        {
            ...
            ui->labelCoinControlChangeLabel->setText(tr("WARNING: Invalid Dravite address"));
        }
        else
        {
